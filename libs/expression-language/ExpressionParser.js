...
const convertKeys = (converter) => (obj) => {
  const newKeys = Object.keys(obj).map(
    (key) => obj.hasOwnProperty(key) ? [key, converter(key)] : null
  ).filter((val) => val != null);
...
class ExpressionParser {
  constructor(options) {
    ...
    if (this.options.isCaseInsensitive) {
      ...
      this.options.PRECEDENCE = this.options.PRECEDENCE.map(
        arr => arr.map(
          val => val.toUpperCase()
        )
      );
    ...
    this.symbols = {};
    this.options.SYMBOLS.forEach(symbol => {this.symbols[symbol] = symbol});
  }

  resolveCase(key) {
    return this.options.isCaseInsensitive 
      ? key.toUpperCase() : key;
  }
  ...
  evaluateRpn(stack, infixer, prefixer, terminator) {
    let lhs, rhs;
  ...
  rpnToExpression(stack) {
    const infixExpr = (term, lhs, rhs) => 
      this.options.GROUP_OPEN + 
      lhs + this.options.SEPARATOR + term + this.options.SEPARATOR + rhs + 
      this.options.GROUP_CLOSE
    ;

    const prefixExpr = (term, rhs) => 
      (this.isSymbol(term) ? term : term + this.options.SEPARATOR) +
      this.options.GROUP_OPEN + rhs + this.options.GROUP_CLOSE
    ;
  ...
  expressionToThunk(expression) {
    return this.rpnToThunk(this.expressionToRpn(expression));
  }

  expressionToValue(expression) {
